...
- (void)setupClientListenSocket {
    ...
    NSError *error = nil;
    [self.cSocket acceptOnPort:ECOClientSockeListenPortNumber error:&error];
    if (error) {
...
- (void)startListening {
    ...
    NSError *error = nil;
    BOOL isAccept = [self.mSocket acceptOnPort:ECOSocketAcceptPortNumber error:&error];
    if (error) {
        NSLog(@"Socket Listen error:%@", error);
...
- (void)connectToIPAddress:(NSString *)ip {
    ...
    GCDAsyncSocket *socket = [[GCDAsyncSocket alloc] initWithDelegate:self delegateQueue:dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0)];
    NSError *error = nil;
    BOOL connected = [socket connectToHost:ip onPort:ECOSocketAcceptPortNumber error:&error];
    if (connected) {
        [self p_lock];
        [self.sockets addObject:socket];
        [self p_unlock];
...
- (void)autoConnectToClientIPAddress:(NSString *)ipAddress {
    ...
    GCDAsyncSocket *socket = [[GCDAsyncSocket alloc] initWithDelegate:self delegateQueue:dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0)];
    NSError *error = nil;
    BOOL connected = [socket connectToHost:ipAddress onPort:ECOClientSockeListenPortNumber error:&error];
    if (connected) {
...
#else
    if ([self.delegate respondsToSelector:@selector(channel:device:didChangedAuthState:)]) {
        BOOL isAuthed = responseType == ECOAuthorizeResponseType_AllowAlways;
        [self.delegate channel:self device:device didChangedAuthState:isAuthed];
    }
#endif
...
//    NSLog(@"%s",__func__);
    ...
    if (tag == ECOSocketDataTag_Authorization) {
        ...
        BOOL isAuthed = tempDevice.authorizedType == ECOAuthorizeResponseType_AllowAlways;
#if TARGET_OS_IPHONE
        ...
        if (tempDevice.showAuthAlert) {
            ...
            if ([self.delegate respondsToSelector:@selector(channel:device:willRequestAuthState:)]) {
                [self.delegate channel:self device:deviceInfo willRequestAuthState:isAuthed];
            }
        }else{
            deviceInfo.authorizedType = tempDevice.authorizedType;
            //连接到新设备
            if ([self.delegate respondsToSelector:@selector(channel:device:didChangedAuthState:)]) {
                [self.delegate channel:self device:deviceInfo didChangedAuthState:isAuthed];
            }
        }
#endif
#if TARGET_OS_OSX
        ...
        if (deviceInfo.uuid.length > 0) {
            NSString *uniId = [NSString stringWithFormat:@"%@_%@",deviceInfo.uuid, deviceInfo.appInfo.appId];
            if (isAuthed && ![self.whitelistDevices containsObject:uniId]) {
                [self.whitelistDevices addObject:uniId];
                [self saveWhiteListDevices];
            }else if (!isAuthed && [self.whitelistDevices containsObject:uniId]){
                [self.whitelistDevices removeObject:uniId];
                [self saveWhiteListDevices];
            }
        }
